...
test("multiple independent subscribers", () => {
  ...
  expect(s1).toHaveBeenLastCalledWith({ state: { name: "off" }, event: undefined });
  expect(s2).not.toHaveBeenCalled();

  toggle.send({ type: "TOGGLE" });
  expect(s1).toHaveBeenCalledTimes(2);
  expect(s1).toHaveBeenLastCalledWith({ state: { name: "on" }, event: { type: "TOGGLE" } });
  expect(s2).not.toHaveBeenCalled();

  const u2 = toggle.subscribe(s2);
  expect(s1).toHaveBeenCalledTimes(2); // still
  expect(s2).toHaveBeenCalledTimes(1);
  expect(s2).toHaveBeenLastCalledWith({ state: { name: "on" }, event: undefined });

  toggle.send({ type: "TOGGLE" });
  expect(s1).toHaveBeenCalledTimes(3);
  expect(s1).toHaveBeenLastCalledWith({ state: { name: "off" }, event: { type: "TOGGLE" } });
  expect(s2).toHaveBeenCalledTimes(2);
  expect(s2).toHaveBeenLastCalledWith({ state: { name: "off" }, event: { type: "TOGGLE" } });
