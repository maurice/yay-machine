...
We describe a state-machine's behaviour - it's potential [state types](../state/), [event types](../events/), [transitions](../transitions/) and [side-effects](../side-effects/) - with `defineMachine<StateType, EventType>({ /* ... */ })`, like so
