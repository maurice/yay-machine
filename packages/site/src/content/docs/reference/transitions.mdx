...
We will expand on the "connection machine" example we used when explaining [states](../state/) and [events](../events/), to illustrate various transitions.
...
<StateChart
  states={['disconnected', 'connecting', 'connected', 'reattemptConnection', 'connectionError']}
  transitions={[
    { from: 'disconnected', to: 'connecting', label: 'CONNECT' },
    { from: 'connecting', to: 'connected', label: 'CONNECTED' },
    { from: 'connecting', to: 'connectionError', label: 'ERROR (invalid auth)' },
    ...
    { from: 'reattemptConnection', to: 'connecting', label: '(after random delay ) CONNECT' },
    { from: 'reattemptConnection', to: 'connectionError', label: "(reached max connection attempts)" },
...
const machine = defineMachine<State, Event>({
  states: {
    [stateName]: {
      always: { /* a single transition */ },
      // OR 
      always: [ /* multiple potential transitions using `when()` */ ] ,
    },
  },
  // ...
});
...
For *homogenous* state-data you also have the option to set `enableCopyDataOnTransition: true` to avoid some boilerplate. See the [states](../state/) documentation for a longer discussion.
...
Transitions MAY define an [`onTransition()` side-effect](../side-effects#transition-ontransition) function that is called if and when the transition is taken.
