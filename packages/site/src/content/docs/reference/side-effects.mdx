...
<Aside type="tip">
 See the section on [`reenter: false` in the transitions documentation](../transitions/#reenter-false) if you need to keep `onEnter()` side-effects alive while transitioning from a state back to the same state.
 </Aside>
