...
Events represent the things that affect the [state of the machine](../state/) as it runs.
...
* generate state data for the next state [in a transition](../transitions/)
* determine which [conditional transition](../transitions/#conditional-transitions-when) to take
...
connection.subscribe(({ state, event }) => {
  console.log('now in state %s, triggered by', state.name, event /* ConnectionEvent | undefined */);
})
```
