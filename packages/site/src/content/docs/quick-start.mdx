...
<Code lang="bash" code="npm add yay-machine" class="install-sh" mark={["yay-machine"]} />
...
### Outline States, Events and Transitions

First, let's define all the **states**, **events** and **transitions** of the state-machine first. Let's ignore *state-data* and *event-payloads* right now.
...
<Code code={atmMachineOutline} lang="typescript" title="atmMachine.ts"/>

### Try visualizing it

Drawing the state-chart might help to see what looks correct, and what is invalid or missing.

import StateChart from '../../components/StateChart/StateChart.astro';

<FullScreen client:load>
<StateChart
  states={['waitingForCard', 'readingCard', 'serviceMenu', 'enterPin', 'enterAmount', 'validateWithdrawal', 'dispenseCash', 'ejectCard']}
  transitions={[
    ...
    { from: 'enterAmount', to: 'enterAmount', label: 'AMOUNT_ENTERED (invalid)' },
    ...
    { from: 'validateWithdrawal', to: 'ejectCard', label: 'INCORRECT_PIN (too many attempts)' },
...
```typescript ins={11-20} {35-50} ins={55-64} ins="enableCopyDataOnTransition: true" 
...
const atm = atmMachine.newInstance({ 
  initialState: { 
    name: "waitingForCard",
    cardReader,
    cashDispenser,
